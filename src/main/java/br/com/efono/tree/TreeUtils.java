...
public class TreeUtils {
     ...
     *
     * Only words that are in the <code>originalCases</code> list will be in the <code>words</code> list.
     *
     * @param node The root node.
     * @param sortedCases A list with originalCases in the right sequence.
     * @param originalCases The user cases.
     */
    public static void buildSequenceOrder(final Node<String> node, final List<KnownCase> sortedCases,
        final List<KnownCase> originalCases) {
        // Esse metodo serve apenas para saber a ordem de inserção na árvore das palavras que estão na lista de casos.
         ...
         * list.
         */
        KnownCase entry = originalCases.stream().filter(c -> c.getWord().equalsIgnoreCase(val)).findFirst().orElse(null);
        if (entry != null) {
            sortedCases.add(entry);
        }

        if (entry == null || entry.isCorrect()) {
            buildSequenceOrder(node.getRight(), sortedCases, originalCases);
            buildSequenceOrder(node.getLeft(), sortedCases, originalCases);
        } else {
            buildSequenceOrder(node.getLeft(), sortedCases, originalCases);
            buildSequenceOrder(node.getRight(), sortedCases, originalCases);
        }
