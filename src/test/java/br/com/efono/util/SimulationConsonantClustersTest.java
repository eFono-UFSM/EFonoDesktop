...
public class SimulationConsonantClustersTest {
    ...
    public void testRun() {
        ...
        System.out.println("testRun - inferred not reproduced and in target words");
        /**
        ...
        System.out.println("testRun - inferred not reproduced and not in target words");
        /**
     ...
     */
    @Test
    public void testRunOnlyClustersInTargetWords() {
        ...
        System.out.println("testRun - inferred not reproduced and not in target words - only considering inferred phonemes that are in target words");
        /**
         * We inferred that the child would be capable of reproduce this phoneme, but she wasn't. Besides that, we are
         * not capable of invalidated the inference, because we can't known if the wasn't capable of reproduce it
         * because she really can't or because the assessment doesn't offer this possibility, by not containing a target
...
//        assertEquals(expectedInferredNotReproducedNotInTargetWords.size(), resultInferredNotReproducedNotInTargetWords.size());
        ...
        System.out.println("testRun - inferred that are in target words");
        List<Phoneme> expectedInferredInTargetWords = Arrays.asList(
                new Phoneme("pl", Phoneme.POSITION.OCI),
                new Phoneme("fɾ", Phoneme.POSITION.OCI)
        );
        List<Phoneme> resultInferredPhonemesInTargetWords = info.getInferredPhonemesInTargetWords();
        assertEquals(expectedInferredInTargetWords.size(), resultInferredPhonemesInTargetWords.size());
        assertTrue(expectedInferredInTargetWords.containsAll(resultInferredPhonemesInTargetWords));
    }
